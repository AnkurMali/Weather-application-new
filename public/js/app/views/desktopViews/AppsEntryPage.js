    ...
    getLocation: function () {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function (position) {
                   ...
                   mycityData.fetch({
    		        url: "http://api.openweathermap.org/data/2.5/forecast/daily?lat="+position.coords.latitude+"&lon="+position.coords.longitude+"&cnt=14"+"mode=json",
	    	        success: function() { 
    ...
    showCityData: function () {
       ...
       // });
        ...
        $.each(city_names,function(i,city_name){
            that.cityData.fetch({
                url: "http://api.openweathermap.org/data/2.5/forecast/daily?q="+ city_name +"&cnt=14&APPID=32e54c52a5dbc89866056c0d07cc567d",
                  success: function() { 
                      $(".weatherInfoContainer").append(_.template(cityWeatherTemplate, {data:that.cityData.toJSON()}));
                }
            });
